...
## A reference to the AudioStreamPlayer for playing walking sounds.
...
## Enable or disable walking sound effects.
@export var walking_sound_enabled : bool = true
## The volume of the walking sound (0.0 to 1.0).
@export_range(0.0, 1.0) var walking_sound_volume : float = 0.7
## The AudioStream resource for walking sounds.
...
var is_playing_walking_sound : bool = false
...
var ineractable_node : Node3D = null :
	set(value):
		ineractable_node = value
		if interact_hint:
...
#region Words Variables

@onready var user_interface: Control = %UserInterface
var found_word_scene: PackedScene = load("uid://biokq55rpcv5u")
@onready var result_text_panel: ResultTextPanel = %ResultTextPanel
...
func _ready():
	...
	result_text_panel.visible = false
...
func handle_interaction():
	if Input.is_action_just_pressed("interaction"):
		if ineractable_node and ineractable_node.has_method("interact"):
			ineractable_node.interact()
			
func remove_interaction():
	ineractable_node = null
	interact_hint.visible = false
...
func handle_walking_sound(is_moving):
	if not walking_sound_enabled or not AUDIO_PLAYER or not walking_sound:
		return
	
	if is_moving and is_on_floor():
		if not is_playing_walking_sound:
			# Start playing walking sound
			AUDIO_PLAYER.stream = walking_sound
			AUDIO_PLAYER.volume_db = linear_to_db(walking_sound_volume)
			AUDIO_PLAYER.play()
			is_playing_walking_sound = true
		
		# Adjust playback speed based on movement state
		match state:
			"normal", "crouching":
		...
		if is_playing_walking_sound:
			# Stop walking sound
			AUDIO_PLAYER.stop()
			is_playing_walking_sound = false
...
func word_found(word: String) -> void:
	var found_word_node: Label = found_word_scene.instantiate()
	found_word_node.text = word
	
	# Get the current word index from GameManager
	var word_index = GameManager.found_words.size() - 1
	
	# Divide screen width equally among all required words
	var screen_width = user_interface.size.x
	var total_required_words = GameManager.required_words.size()
	var column_width = screen_width / total_required_words
	var final_x_position = word_index * column_width  # Center word in column
	var final_position = Vector2(final_x_position, 40)
	
	# Start at center of screen with alpha 0
	var screen_center = user_interface.size / 2
	found_word_node.modulate.a = 0.0
	
	user_interface.add_child(found_word_node)
	found_word_node.global_position = screen_center - found_word_node.size / 2
	
	# Create tween for animation
	var tween : Tween = create_tween()
	
	# Fade in at center
	tween.tween_property(found_word_node, "modulate:a", 1.0, 1.5)
	
	# Move to final position
	tween.tween_property(found_word_node, "position", final_position, 1.8)
	tween.set_ease(Tween.EASE_OUT)
	tween.set_trans(Tween.TRANS_QUART)


func _on_focus_input_button_pressed() -> void:
	Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
